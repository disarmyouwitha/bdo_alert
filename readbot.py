...
class ReadBot(object):
    ...
    def __init__(self, digits_only=False, ocr_engine='tesseract', pdf_converter='ghostscript'):
    ...
    def run_ocr(self, input_file):
        ...
        if self.ocr_engine == 'tesseract':
            ocr_result = engine.tesseract(input_file, self.digits_only)

        return ocr_result


class OcrEngine(object):
    def tesseract(self, input_file, digits_only):

        try:
            if digits_only:
                subprocess.call(['C:/Program Files (x86)/Tesseract-OCR/tesseract.exe', input_file.name, tempfile.gettempdir() + '/ocr_result', 'digits'])
            else:
                subprocess.call(['C:/Program Files (x86)/Tesseract-OCR/tesseract.exe', input_file.name, tempfile.gettempdir() + '/ocr_result'])      
        except OSError:
